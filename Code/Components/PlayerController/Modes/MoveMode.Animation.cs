...
partial class MoveMode
{
	...
	protected virtual void OnUpdateAnimatorVelocity( SkinnedModelRenderer renderer )
	{
		...
		// skid
		{
			var skidAmount = 1.0f; // multiplier for moving skid
			var pushSkidAmount = 1.0f; // multiplier for sliding down a slope when standing still
			var skidDelay = 0.4f; // in seconds, longer means bigger gap between the velocities, more skidding
	...
	protected virtual void OnUpdateAnimatorState( SkinnedModelRenderer renderer )
	{
		...
		var duck = Controller.Headroom.Remap( 50, 0, 0, 0.5f, true );
	...
	/// <summary>
	/// Set animation parameters on <paramref name="renderer"/> to look towards <see cref="PlayerController.EyeAngles"/>.
	/// </summary>
	...
	{
		var targetAngle = Rotation.FromYaw( Controller.EyeAngles.yaw );
